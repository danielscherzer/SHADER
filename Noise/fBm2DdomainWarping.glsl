...
void main() 
{
	...
	color = mix(color, vec3(0 , 0 , 0.16), clamp(fBmCoord, 0.0, 1.0)); // step 1
	color = mix(color, vec3(0.67, 1, 1), clamp(fBmfBm,0.0,1.0)); // step 2

	color = (f * f * f + 0.2 * f * f + 0.5 * f) * color; // step 3
}
