...
float noise( in vec3 x )
{
	...
	vec2 rg = texture2D( tex, (uv+ 0.5)/256.0 ).yx;
#else
	vec2 uv = (p.xy+vec2(37.0,17.0)*p.z);
	vec2 rg1 = texture2D( tex, (uv+ vec2(0.5,0.5))/256.0 ).yx;
	vec2 rg2 = texture2D( tex, (uv+ vec2(1.5,0.5))/256.0 ).yx;
	vec2 rg3 = texture2D( tex, (uv+ vec2(0.5,1.5))/256.0 ).yx;
	vec2 rg4 = texture2D( tex, (uv+ vec2(1.5,1.5))/256.0 ).yx;
...
float noise( in vec2 x )
{
	...
	return texture2D( tex, (uv+118.4)/256.0 ).x;
}
...
//time =35.0;
    ...
    if( t>0.0 )
	{
		...
		vec3 bn = -1.0 + 2.0*texcube( tex, 3.0*pos/4.0, nor ).xyz;
		...
		col = texcube( tex1, 0.5*pos, nor ).xyz;
		col = col*(0.2+0.8*texcube( tex2, 4.0*vec3(2.0,8.0,2.0)*pos, nor ).x);
		vec3 verde = vec3(1.0,0.9,0.2);
		verde *= texture2D( tex2, pos.xz ).xyz;
		col = mix( col, 0.8*verde, hh );
		
		float vv = smoothstep( 0.0, 0.8, nor.y )*smoothstep(0.0, 0.1, pos.y-0.8 );
		verde = vec3(0.2,0.45,0.1);
		verde *= texture2D( tex2, 30.0*pos.xz ).xyz;
		verde += 0.2*texture2D( tex2, 1.0*pos.xz ).xyz;
		vv *= smoothstep( 0.0, 0.5, texture2D( tex2, 0.1*pos.xz + 0.01*nor.x ).x );
